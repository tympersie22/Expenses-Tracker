export function productionConfigIssues() {
  ...
  const issues: string[] = [];
  try {
    if (!process.env.APP_ORIGIN || new URL(process.env.APP_ORIGIN).protocol !== "https:") issues.push("APP_ORIGIN_HTTPS");
  } catch { issues.push("APP_ORIGIN_HTTPS"); }
  ...
  if (!process.env.MONITORING_WEBHOOK_URL) issues.push("MONITORING");
  return issues;
}
